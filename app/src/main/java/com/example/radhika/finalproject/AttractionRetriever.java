...
public class AttractionRetriever {
    static URLConnection connection;

    public static void GetPlaces(AttractionRetriever.OnPlaceListRetrievedListener listener) {
    ...
    public static class GetPlacetask extends AsyncTask<Void, Void, List<Place>> {
        ...
        protected List<Place> doInBackground(Void... params) {
            try {
                ...
                if (map_num == 1) {
                    //paris
                    connection = new URL("https://maps.googleapis.com/maps/api/place/nearbysearch/json?location=48.8686,2.3002&radius=800&types=point_of_interest&key=AIzaSyALd_KbyBkJxy4LrayDU3_nPTSYwZDynIY").openConnection();

                }
                if (map_num == 2) {
                    //berlin
                    connection = new URL("https://maps.googleapis.com/maps/api/place/nearbysearch/json?location=52.5200,13.4050&radius=800&types=point_of_interest&key=AIzaSyALd_KbyBkJxy4LrayDU3_nPTSYwZDynIY").openConnection();
                }
                if (map_num == 3){
                    //rome
                    connection = new URL("https://maps.googleapis.com/maps/api/place/nearbysearch/json?location=41.9028,12.4964&radius=800&types=point_of_interest&key=AIzaSyALd_KbyBkJxy4LrayDU3_nPTSYwZDynIY").openConnection();

                }
                if (map_num == 4)
                {
                    //ams
                    connection = new URL("https://maps.googleapis.com/maps/api/place/nearbysearch/json?location=52.3702,4.8952&radius=800&types=point_of_interest&key=AIzaSyALd_KbyBkJxy4LrayDU3_nPTSYwZDynIY").openConnection();
                ...
                if (map_num == 5)
                {
                    connection = new URL("https://maps.googleapis.com/maps/api/place/nearbysearch/json?location=" +MapsActivity.lat +"," +MapsActivity.lng+ "&radius=800&types=point_of_interest&key=AIzaSyALd_KbyBkJxy4LrayDU3_nPTSYwZDynIY").openConnection();
